...
try:
    ...
    import socket, json, ipaddress, zlib, itertools, gzip, struct, getpass, lockis
...
def certs(main_server): # check server status and ssl certificate
    global tunnel_cert
    try:
        post = rpost(f"https://{main_server}:5569/c/status", json_data={"id": 0}, timeout=10)["response"]
        if post["server"] == "ok" and post["tls_version"] in ("TLSv1.3", "TLSv1.2"):
            return True
        else:
            return False
...
def mtunn_path(): # config file locations
    ...
    elif platform.system().lower() in ("openbsd", "freebsd", "netbsd", "darwin", "linux"):
...
class Client(SockRunable):
    ...
    def _run_ucheck(self):
        ...
        while self._running:
            try:
                post = rpost(f"https://{self.primary}:5569/c/version", json_data={"type": "latest-stable"}, timeout=10)["response"]
    ...
    if args.version:
        ...
        try:
            ...
            if buffer < 131072:
                ...
                raise SystemExit
            if buffer > 2097152:
                print_s(f"\033[01;36m"+str(time.strftime("%H:%M:%S"))+f"\033[0m [\033[01;31mERROR\033[0m] too big buffer, maximum 2 MB")
                raise SystemExit
        except (NameError, ValueError, KeyError):
